...
import { MapPin, BedDouble, Ruler, Building } from 'lucide-react';
  ...
  return (
    <Card className={cn(
      ...
      <div className={cn("relative flex-shrink-0", isListView ? "w-1/3" : "w-full")}>
        ...
        <Badge variant="secondary" className="absolute top-4 left-4 z-10">{property.projectName}</Badge>
      </div>

      <CardContent className="p-4 flex flex-col flex-grow">
         <div className="flex-grow">
            <p className="text-2xl font-bold text-primary mb-2">{formatCurrency(property.price, 'INR')}</p>
            ...
            <div className={cn("mt-4 pt-4 border-t gap-4 text-sm", isListView ? "grid grid-cols-2" : "grid grid-cols-2")}>
                 ...
                 <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{property.builtUpArea > 0 ? `${property.builtUpArea} sqft` : 'N/A'}</span>
                </div>
                 <div className="flex items-center gap-2 text-muted-foreground">
